...
import urllib, urllib2, xbmcplugin, xbmcaddon, xbmcgui, string, htmllib, os, platform, re, xbmcplugin, sys, datetime
import requests  
import urlparse
import HTMLParser
...
def cats(url):
	...
	for url, title in match[:-2]:
	    ...
	    image = defaultimage
	    add_directory2("[COLOR white]"+title+"[/COLOR]",url,mode,defaultfanart,defaultimage,plot='')
...
def collections(url):
	...
	for url, title in zip(urls, titles):
	    ...
	    add_directory2("[COLOR white]"+title+"[/COLOR]",url,18,defaultfanart,defaultimage,plot='')
        xbmcplugin.endOfDirectory(addon_handle)
...
def years(url):
	...
	for year in years[1:]:
	    ...
	    add_directory2("[COLOR white]"+year+"[/COLOR]",url,13,defaultfanart,defaultimage,plot='')
        xbmcplugin.endOfDirectory(addon_handle)
...
def months(url):
	...
	for month in months[1:]:
	    ...
	    add_directory2("[COLOR white]"+month+"[/COLOR]",url,14,defaultfanart,defaultimage,plot='')
        xbmcplugin.endOfDirectory(addon_handle)
...
def episodes(url):
	...
	for day, guest, episode in zip(days, guests, episodes):
	    title ="[COLOR red]"+day+": [COLOR white]"+ guest+"[/COLOR]"
...
def atoz(url): 
	    ...
	    url = 'https://charlierose.com/guests?char=' + name + '&listing_type=az'
            add_directory2("[COLOR white]"+name+"[/COLOR]",url,16,defaultfanart,defaultimage,plot='')    
...
def guests(url):
	...
	for guest, link in zip(guests, links):
	    url = 'https://charlierose.com' + link
            add_directory2("[COLOR white]"+guest+"[/COLOR]",url,17,defaultfanart,defaultimage,plot='')    
...
def gstreams(url):
	...
	for link, date, image in zip(links, dates, images):
	    ...
	    add_directory2("[COLOR red]"+date+"[/COLOR]",url,30,defaultfanart,image,plot='')
        #xbmc.executebuiltin("Container.SetViewMode("+str(confluence_views[3])+")")
        xbmcplugin.endOfDirectory(addon_handle)
...
def cvideos(url):
	...
	for item in soup:
	    ...
	    title = "[COLOR red]"+airdate+" - [COLOR white]"+re.compile('<b>(.+?)</b>').findall(str(title))[-1]+"[/COLOR]"
...
def videos(url):
	...
	for item in soup:
	    ...
	    image = item.find('img')['src']
	    print url
	    add_directory2(title+"[/COLOR]",url,30,defaultfanart,image,plot='')#; i = i + 1
	try:next = 'https://charlierose.com' + re.compile('next" href="(.+?)"').findall(response)[-1]
	except IndexError:
	    return
...
def streams(name,url):
	...
	url="plugin://plugin.video.bobtv/?url="+source+"&name=Charlie Rose Site&mode=1000&title="+name;xbmc.executebuiltin('XBMC.RunPlugin(%s)' % url) 
...
def out(what2say):
    mylog=xbmc.translatePath(os.path.join('special://logpath','ron.log'))
    f= open(mylog,"a+")
