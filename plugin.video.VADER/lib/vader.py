...
blockedUsers = ['beastTrial', 'black', 'coralblair', 'LeeTVbro']
...
class vader(object):
    def __init__(self, popup=True):
        ...
        self.embedded = False
        self.caps = self._fetch('caps', capsUrl)
        if self.caps != None:
        ...
        if self.embedded == True:
            ...
            self.neededPath = mainPath + '/code_cache/a.out'
            utils.ensure_dir(self.neededPath)
            if not os.path.exists(self.neededPath):
                if os.path.exists(os.path.join(ADDONPATH, 'a.out')):
                    copyfile(os.path.join(ADDONPATH, 'a.out'), self.neededPath)

            xbmc.sleep(100)
            if os.path.exists(self.neededPath):
                st = os.stat(self.neededPath)
                os.chmod(self.neededPath, st.st_mode | stat.S_IEXEC)

                p = subprocess.Popen([self.neededPath], stdout=subprocess.PIPE)
                out, err = p.communicate()
                self.username = out.split(':')[0].strip()
                self.password = out.split(':')[1].strip()
            else:
                utils.showNotification(ADDONNAME, 'Corrupted Install, missing components, please re-install.')
        else:
    ...
    def _fetch(self, action, base_url=None, od=True, force=False):
                ...
                if timeNow - fileTime > 1800:
                    ...
                    with open(cachePath, 'r') as cacheFp:
                        ...
                        try:
                            ...
                            if self.embedded != True:
                                plugintools.log('getting from vader api ' + url)
                            readString = self.session.get(url).text
                            with open(cachePath, 'w') as cacheFp:
                                cacheFp.write(readString)
            else:
    ...
    def build_stream_url(self, stream, extension='ts', base='live'):
        ...
        if self.embedded == True:
            ...
            active_cons = int(self.user_info['user_info']['active_cons'])
            max_cons = int(self.user_info['user_info']['max_connections'])

            if active_cons < max_cons:
                timeNow = time.time()
                ...
                utils.showNotification(ADDONNAME,
                                       'Your account has used the max connections')
    ...
    def getAuth(self):
        ...
        try:
            action = 'auth'
            url = 'http://{apiEndpoint}/player_api.php?username={username}&password={password}'.format(
                username=self.username, password=self.password, apiEndpoint=self.apiEndpoint)
            json = self._fetch(action, base_url=url)
            return json
        except Exception as e:
    ...
    def authorise(self):
        ...
        if self.user_info != None:
            if self.user_info['user_info']['auth'] == 1:
                ...
                if self.embedded == True:
                    if os.path.exists(os.path.join(ADDONPATH, 'a.out')):
                        utils.delete(os.path.join(ADDONPATH, 'a.out'))

                self.authString = self.user_info['user_info']['status']
    ...
    def get_playlink_archive(self, stream_id, playTime, duration='120'):
        ...
        if self.embedded == True:
            ...
            diffTime = timeNow % 3600
            tokenTime = timeNow - diffTime
            tokenString = self.username + self.password + str(stream_id) + str(tokenTime)
            token = hashlib.md5(tokenString).hexdigest()

            url = 'http://{apiEndpoint}/boxRedir?token={token}&stream={stream_id}&duration={duration}&start={start}&type=timeshift&plugin={plugin}'.format(
                token=token, stream_id=stream_id, duration=duration, start=playTime, plugin=ADDONID,
                apiEndpoint=self.apiEndpoint)
            ...
            url = 'http://{apiEndpoint}/streaming/timeshift.php?username={username}&password={password}&stream={stream_id}&duration={duration}&start={start}'.format(
        ...
        if self.pvriptvsimple_addon != None:
            ...
            if self.mc_timezone_enable == False:
                if '16' in self.kodi_version or '15' in self.kodi_version:
                    ...
                    utils.log('Timezone override disabled. Using API timezone of {0}'.format(str(offset)))
                if '17' in self.kodi_version:
