#added by ron
import re,xbmcvfs,urllib2
from bigoldboy import *
from os import listdir
from os.path import isfile, join

offset=7200 #corrects Catchup being off by two hours
...
def cname(channelnumber): # wrote this to search multichannels for vader catchup
    ...
    for stationname,group,url in contents:
            if channelnumber == url: 
                if "Channel #" in stationname:
                    stationname = "Sports "+stationname
                return stationname
...
def show_catchup_mc():
    ...
    for epgitem in mcArchiveEvents:
        ...
        duration = int((stopTime - startTime)/60) + 300

        if (startTime > backwardTimeTs and startTime < timeNow):
            displayTime = datetime.fromtimestamp(int(startTime)).strftime('%d.%m - %H:%M') 
            displayTime = datetime.fromtimestamp(int(startTime)-offset).strftime('%I:%M %p') #changes to AM/PM and adds offset
            displayDay = datetime.fromtimestamp(int(startTime)-offset).strftime('%A') #figure out the day of week and spell it out
            bannerman="[COLOR blue]*************************  "+ displayDay +"  *************************[/COLOR]" #banner for day      
            utc = pytz.utc
            playTime = datetime.fromtimestamp(int(startTime), tz=utc).strftime('%Y-%m-%d:%H-%M')
            finalTitle = displayTime + " - " + title
            if "BELLATOR" in title.upper() or "UFC" in title.upper() or "TORONTO" in title.upper() or "MMA" in title.upper() or "VictoryFC" in title.upper(): #Highlight My Programs: 
                finalTitle = "[COLOR red]" + displayTime + "[COLOR yellow][B] - " +    title +"[/B][/COLOR]"
            else: 
                finalTitle = "[COLOR red]" + displayTime + "[COLOR white] - " +    title +"[/COLOR]"
            if whatday != displayDay:
                addDirectoryItem(plugin.handle, plugin.url_for(play_archive, stream_id, playTime, title, str(duration)), ListItem(bannerman), False)           
            whatday = displayDay
        ...
        if category_id == 'all':
            ...
            try:                
                ...
                for streamObj in streams:
                    ...
                    title = str(title.encode('utf-8').decode('ascii', 'ignore'))
                    utils.log(title)
                    bobchannel=str(re.compile('rondyer/(.+?).ts').findall(chanUrl)) #parse 
                    bobchannel=bobchannel.replace(',',"").replace('[',"").replace(']','').replace("'",'') # clean it up 
                    boburl="plugin://plugin.video.bobtv/?url="+str(bobchannel)+"&name="+chanName+"&mode=990&title="+title
                    adddir(boburl, title, icon)
...
def show_livetv_all():
    ...
    for streamObj in streams:
        ...
        if category_id not in vaderClass.filter_category_list_id:
            ...
            bobchannel=str(re.compile('rondyer/(.+?).ts').findall(chanUrl)) #parse 
            bobchannel=bobchannel.replace(',',"").replace('[',"").replace(']','').replace("'",'') # clean it up 
            boburl="plugin://plugin.video.bobtv/?url="+str(bobchannel)+"&name="+displayname+"&mode=990&title="+title
            adddir(boburl, title, icon)
            #plugintools.add_playable(title=title, url=chanUrl,
             #                        thumbnail=icon, plot='', isPlayable=True, folder=False)
...
def play_archive_adjusted(stream_id, playTime, title, duration):
    progress_create("Search Cloud for Show",str.center(title,119-len(title),' '))          
    utils.log(playTime)
    ...
    # create temp file to move the data to bobtv
    xferfile=xbmc.translatePath(join('special://logpath','vaderxfer'))
    f=open(xferfile,"w")
    f.write(url)
    f.close
    
    catchuplog=xbmc.translatePath(join('special://logpath',title.replace(":","")+'.m3u8'))
    f=open(catchuplog,"a+")
    f.write("#EXTM3U""\n")
    f.write("#EXTINF:-1,"+str(title)+" ("+stream_id+") @ "+str(playTime)+"\n")
    f.write(str(url)+"\n")
    f.close
    
    bob="plugin://plugin.video.bobtv/?url=vaderxfer&name="+cname(stream_id)+"&mode=991&title="+title
    xbmc.executebuiltin('XBMC.RunPlugin(%s)' % bob)    
    return
...
def show_catchup_listing(stream_id):
    ...
    timeNow     = int(timeNow)
    showNames = []
    displayDay="" ; whatday = "1"
    for epgitem in      jsondata:
        title = epgitem ['title']
        title = base64.b64decode(title)
        x=title.find("[")
        if x>1: title=title[:x-1]
        startTime = int(epgitem['start_timestamp'])
        stopTime = int(epgitem['stop_timestamp'])
        duration = int((stopTime-startTime)/60) + 5
        ...
        if(startTime > backwardTimeTs and startTime < timeNow):
            if vaderClass.group_by_name == True:
                if title not in showNames:
                    ...
                    displayTime = datetime.fromtimestamp(int(startTime)).strftime('%d.%m - %H:%M')
                    displayTime = datetime.fromtimestamp(int(startTime)-offset).strftime('%I:%M %p')
                    displayDay = datetime.fromtimestamp(int(startTime)-offset).strftime('%A')
                    utc = pytz.utc
                    if '[New!]' in title:
                        finalTitle = displayTime + " - " + title
                    else:
                ...
                finalTitle = "[COLOR red]" + displayTime + "[COLOR white] - " +    title +"[/COLOR]"
                if whatday != displayDay:
                    bannerman="[COLOR blue]*************************  "+ displayDay +"  *************************[/COLOR]" 
                    addDirectoryItem(plugin.handle, plugin.url_for(play_archive, stream_id, playTime, title, str(duration)), ListItem(bannerman), False)
                    whatday = displayDay
